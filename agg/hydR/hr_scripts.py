...
class RastRun(Model):
    ...
    def build_difrlays(self, #generate a set of delta rasters and write to the catalog
        ...
        for resolution, d0 in lay_lib.items():
            d = dict()
            assert isinstance(d0, dict), 'got bad type on reso=%i: %s'%(resolution, type(d0))
    ...
    def build_resdx(self, #just combing all the results
        ...
        first = True
        d = dict()
        for dki in {e for k,v in phase_d.items() for e in v }:
            dx = self.retrieve(dki).sort_index(sort_remaining=True)
    ...
    def build_resdxfp(self, #export everything to the library and write a catalog
        ...
        """todo: add filesize"""
        ofp_lib = dict()
        for dki in [v for k,v in {'depth':'drlay_lib', 'diff':'difrlay_lib'}.items() if k in phase_l]:
